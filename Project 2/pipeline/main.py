...
def process_range(start: int, end: int, resume: bool = False, batch_size: int = 1, **kwargs):
    ...
    while idx < len(file_ids):
        ...
        for i in batch_ids:
            ...
            batch_indices.append((i, filename))
        ...
        t_batch = time.time()
        file_nums = ",".join(str(i) for i, _ in batch_indices)
        print(f"\n{'=' * 60}")
        ...
        for tidx, ((i, filename), entities) in enumerate(zip(batch_indices, all_entities)):
            print(f"\n  [{len(results)+1}/{end-start+1}] Processing: {filename}")
            doc_id = str(i)
            out_path_full = os.path.join(OUTPUT_DIR, f"{doc_id}.json")

            try:
                r = _process_with_entities(entities, batch_texts[tidx],
                                           doc_id, **kwargs)
                r["skipped"] = False
                results.append(r)

                with open(out_path_full, "w", encoding="utf-8") as f:
                    json.dump(r.get("entities", []), f, ensure_ascii=False, indent=2)
...
def _process_with_entities(entities: list[dict], input_text: str, doc_id: str,
                           skip_assertion: bool = False, skip_linker: bool = False,
                           skip_position: bool = False, expand_dup: bool = True) -> dict:
    t0 = time.time()
    orig_count = len(entities)
    ...
    elapsed = time.time() - t0
    print(f"  -> {len(entities)} concepts, {elapsed:.1f}s")
    return {"id": doc_id, "entities": len(entities), "ok": True,
            "out": os.path.join(OUTPUT_DIR, f"{doc_id}.json")}
...
def main():
    ...
    parser.add_argument("--batch-size", type=int, default=10, help="Batch N files for entity extraction (default 10)")
